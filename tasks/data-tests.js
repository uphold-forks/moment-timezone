...
function yearTest (year, changes, name) {
	return '\t"autogenerated:' + year + '" : helpers.makeTestYear("' + name + '", [\n\t\t' + changes.join(',\n\t\t') + '\n\t])';
}
...
function population (data, grouped) {
	...
	return '\t"autogenerated:guess" : helpers.makeTestGuess("' + data.name + '", { offset: '
		+ isMostPopulatedInOffset + ', abbr: ' + isMostPopulatedInAbbr + ' })'
		+ testSeparator;
}

/**
 * Reads test file from the disk
 * Finds there non-autogenerated tests
 * And returns them as a string
 * @param fileName
 * @returns {*}
 */
function getTestsToKeep(fileName){
	if (fs.existsSync(fileName)) {
		var fileContents = fs.readFileSync(fileName).toString();
		var exportsMatch = fileContents.match(/exports.*{\n([\s\S]+)\n};/m);
		if (exportsMatch === null) {
			console.warn('Warning: empty tests ' + fileName);
			return '';
		}
		if (exportsMatch.length !== 2) {
			throw new Error('Did not find exports section in ' + fileName);
		}
		var tests = exportsMatch[1].split(testSeparator);
		var testsToKeep = tests.filter(function (testBody) {
			return /"autogenerated:.*?"/.test(testBody) === false;
		});
		if (testsToKeep.length > 0) {
			return testsToKeep.join(testSeparator) + testSeparator;
		} else {
			return '';
		}
	} else {
...
module.exports = function (grunt) {
	grunt.registerTask('data-tests', '8. Create unit tests from data-collect.', function () {
		...
		zones.forEach(function (zone) {
			var fileName = path.join('tests/zones', zone.name.toLowerCase() + '.js');
			var testsToKeep = getTestsToKeep(fileName);
			var data = intro(zone.name) + testsToKeep + population(zone, grouped) + tests(zone)  + '\n};';
